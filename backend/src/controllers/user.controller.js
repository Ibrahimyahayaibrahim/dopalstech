    ...
    if (user) {
      ...
      try {
        await sendEmail({
          ...
          html: messageHtml, 
          text: `Your password is: ${finalPassword}`
        });
...
export const removeFromDepartment = async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(userId, { $pull: { departments: departmentId } });
...
export const migrateStaff = async (req, res) => {
    try {
        ...
        if (!user.departments.includes(newDeptId)) user.departments.push(newDeptId);
        await user.save();
