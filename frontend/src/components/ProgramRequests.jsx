...
import { Check, X, Clock, FileText, DollarSign, Calendar, AlertCircle, Users, Edit3 } from 'lucide-react';
...
const ProgramRequests = ({ searchQuery = '' }) => {
  ...
  // --- FILTERING LOGIC (GLOBAL VISIBILITY FIX) ---
  
  // 1. Master List: Contains ALL programs that match search (No Department Filter Here!)
  const allPrograms = programs.filter(p => 
    p.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    p.createdBy?.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // 2. Action Required: RESTRICTED list (Only items THIS user needs to approve)
  const pendingApprovals = allPrograms.filter(p => {
      if (p.status !== 'Pending') return false;
      
      // Super Admin: Acts on everything
      if (isSuperAdmin) return true;

      // Admin: Acts on their department
      if (isAdmin) {
  ...
  // 3. My Pending: RESTRICTED list (For Staff/Admins to track their OWN requests)
  const myPending = isStaff ? allPrograms.filter(p => p.status === 'Pending' && p.createdBy?._id === user._id) : [];

  // 4. History / All Programs: SHOWS EVERYTHING from ALL departments
  // We just exclude the ones already in "Action Required" or "My Pending" to avoid duplicates in the UI
  ...
  // --- HELPER: PERMISSION CHECK FOR EDIT BUTTON ---
  const canUserEdit = (program) => {
      if (isSuperAdmin) return true;
      
      const pDeptId = program.department?._id || program.department;
      
      // Admin can edit any program in their department
      if (isAdmin && myDeptIds.includes(pDeptId)) return true;

      // Staff can only edit THEIR OWN programs in their department
  ...
  return (
    <div className="space-y-8 animate-enter pb-10">
      ...
      {/* SECTION A: ACTION REQUIRED (Admins & Super Admin) */}
      ...
      {/* SECTION C: GLOBAL HISTORY (Visible to Everyone) */}
      <div>
        <h2 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                  ...
                  <tbody className="divide-y divide-gray-50">
                     {historyList.map(p => (
                        <tr key={p._id} onClick={() => navigate(`/programs/${p._id}`)} className="hover:bg-gray-50/50 cursor-pointer transition-colors group">
                            ...
                            <td className="p-4 text-gray-500 whitespace-nowrap">{formatDate(p.date)}</td>
                            ...
                            <td className="p-4 text-right flex justify-end gap-2">
                                ...
                                {(p.status === 'Approved' || p.status === 'Completed') && (isSuperAdmin || canUserEdit(p)) && (
                                    <button 
                     ...
                     {historyList.length === 0 && (
                        <tr><td colSpan="6" className="p-12 text-center text-gray-400"><p>No programs found.</p></td></tr>
                     )}
                  </tbody>
               </table>
           </div>
...
// --- CARD COMPONENT ---
const ProgramCard = ({ program, onClick, onAction, onEdit, formatDate, canAct, isOwner, canEdit }) => (
    <div onClick={onClick} className={`bg-white p-5 rounded-2xl shadow-sm border relative overflow-hidden group hover:shadow-md transition-all cursor-pointer ${isOwner ? 'border-blue-100' : 'border-amber-100'}`}>
        <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${isOwner ? 'bg-blue-400' : 'bg-amber-400'}`}></div>
